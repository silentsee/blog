...
class Column(object):
    ...
    def __str__(self):
        s = ['< %s: %s %s' % (self.__class__.__name__, self.name, self.default)]
...
class BooleanFiled(Column):
    def __init__(self, **kw):
        if 'default' not in kw:
            kw['default'] = False
        if 'type' not in kw:
            kw['type'] = 'bool'
        super(BooleanFiled, self).__init__(**kw)
...
class Model(dict):
    ...
    def find_first(cls, where, *args):
        ...
        :return cls() or None
        """
        r = db.select_one('select * from %s %s' % (cls.__table__, where), *args)
        return cls(**r) if r else None
...
if __name__ == '__main__':  # 测试时运行
    logging.basicConfig(level=logging.CRITICAL)  # 将日志级别设为debug
