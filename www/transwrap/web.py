...
class HttpError(Exception):
    ...
    def __init__(self, code):
        ...
        self.status = '%d %s' % (code, _RESPONSE_STATUSES[code])#一个字典_RESPONSE_STATUSES存储错误吗以及对应的描述

    def header(self, name, value):
        if not hasattr(self, '_headers'):
            self._headers = [_HEADER_X_POWERED_BY] #Todo
...
_TIMEDELTA_ZERO = datetime.timedelta(0)#Todo

# timezone as UTC+8:00, UTC-10:00

_RE_TZ = re.compile('^([\+\-])([0-9]{1,2})\:([0-9]{1,2})$')#一个正则表达式的对象,用于匹配


class UTC(datetime.tzinfo):#Todo
    '''
    A UTC tzinfo object.
...
#将_RESPONSE_HEADERS大写,_RESPONSE_HEADER_DICT为大小写对应的字典
...
class WSGIApplication(object):
    ...
    def get_wsgi_application(self, debug=False):
        ...
        def wsgi(env, start_response):
            ctx.application = _application #当前app目录
